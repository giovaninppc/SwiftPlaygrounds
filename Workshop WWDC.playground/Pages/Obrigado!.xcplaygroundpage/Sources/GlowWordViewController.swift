...
public class GlowWordViewController : UIViewController {
    ...
    override public func loadView() {
        ...
        timer = Timer.scheduledTimer(timeInterval: 0.2, target: self, selector: #selector(createRandomWord), userInfo: nil, repeats: true)
    }
    ...
    func addLabel(){
        ...
        subtitle.text = "Obrigado! (Em muitas línguas)"
    ...
    @objc func tapHandler(){
        ...
        self.view.addSubview(GlowingEmoji(frame: CGRect(x: CGFloat(X), y: CGFloat(Y), width: CGFloat(size), height: CGFloat(size))))
    }

    /// Cria circulos aleatorios na tela
    ///
    @objc func createRandomWord(){
        ...
        self.view.addSubview(GlowingWord(frame: CGRect(x: CGFloat(X), y: CGFloat(Y), width: CGFloat(size), height: CGFloat(size)), color: RandomColor.randomColor()))
    }
}
